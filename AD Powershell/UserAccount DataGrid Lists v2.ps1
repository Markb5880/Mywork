...
$ButtonX.Add_Click({
    $dataGridView.DataSource = $null
    $tableData = New-Object System.Collections.ArrayList
    $script:row1 = Get-ADUser -Filter * -Properties * -SearchBase "ou=administration,ou=f1,ou=ts tech users,dc=smallhome,dc=local" | Select-Object cn,company,department,lastlogondate,passwordlastset,enabled | Sort-Object cn
    $tableData.AddRange($row1)
    $dataGridView.DataSource=$tableData
    })
