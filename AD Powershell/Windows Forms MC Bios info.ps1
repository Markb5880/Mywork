...
# Load the items and sort the list in Ascending order.
    $computers=get-adcomputer -filter * -SearchBase "CN=Computers,DC=smallhome,DC=local" | where {$_.enabled -eq $true } | select name |sort name
...
# Display an Image on a form
    $image = [System.Drawing.Image]::Fromfile('C:\Users\administrator\Downloads\lotus-notes-icon1.png')     
